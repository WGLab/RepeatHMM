...
def get_gLoc(repeatname, commonOptions):
	...
	if len(infospt)<nece_field_num:
		logging.error("Error: wrong input for the gene of interes: %s whose length is %d less than the expected length %d" % (newinfo, len(infospt), nece_field_num));
		sys.exit(101);
	...
	if not errorstr=='':
		...
		if commonOptions['outlog'] <= M_WARNING: print errorstr, curloc, repeatName
		sys.exit(102);

	return get_Loc1(curloc, commonOptions)

def get_Loc1(curloc, commonOptions):
	curloc[1] = int(curloc[1]); curloc[2] = int(curloc[2])
	retOptions = {}
	...
	elif commonOptions['SeqTech']=="Pacbio": rep_up_down_size = 1000;
	elif commonOptions['SeqTech']=="Nanopore": rep_up_down_size = 1000;
	else: rep_up_down_size = 1000;
	retOptions['gene_start_end'] = [curloc[1]-rep_up_down_size, curloc[2]+rep_up_down_size]
	if retOptions['gene_start_end'][0]<1: retOptions['gene_start_end'] = 1;
...
def insert_n_for_flanking(oldstr, rep_predata, rep_sufdata, repeatFlankLength, repPat, otherinfo):
	...
	if lengthmore>100: lengthmore = 100
	...
	for curp_ind in range(len(twopairs)):
		...
		if align_len==0 and len(curp[1])>0: align_len=len(curp[1])
		elif align_len>len(curp[1]) and len(curp[1])>min_flank_len: align_len=len(curp[1])
		if align_len<min_flank_len: align_len=min_flank_len
		...
		numsame=0; numnot=0; numgap=0; query_start = abegin; query_end = abegin
		for ai in range(abegin, aend):
			if not seq1[ai]=='-': 
				query_end += 1
				if seq1[ai]==seq2[ai]: numsame += 1;
				elif seq2[ai]=='-': numgap += 1
				else: numnot += 1;
			else:
				if seq2[ai]=='-': 
					if cur_M_STAT <= M_ERROR: print 'Fatal!!! both gap', seq1, seq2
				else: numgap += 1
		...
		if align_len>min_flank_len and identical_fraction>0.75: 
			...
			if cur_M_STAT <= M_INFO: print 'xxxxxxxxxx', oldstr;
			detectpos.append([None, None])
			...
			detail2.append(('%d/%d=%.2f for len(%d, %d)' % (numsame, align_len, identical_fraction, len(curp[0]), len(curp[1]))))
			if align_len==0:
				if cur_M_STAT <= M_ERROR:
					print 'Error!!! zero alignment ', curp, aend, abegin
					logging.error('Error!!! zero alignment ' + str(curp)+ ' '+ str(aend) +' '+ str(abegin))
		if cur_M_STAT <= M_INFO:
			print 'info', seq1, detectpos[-1]
			print '    ', seq2, numsame, len(curp[1]), align_len, 
			if align_len>0: print round(numsame/float(align_len), 2)
			else: print ''

	if cur_M_STAT <= M_INFO: print 'seq', oldstr;
	if not (detectpos[1][0]==None and detectpos[0][1]==None): 
		if cur_M_STAT <= M_INFO: print detectpos;
	else: 
		if cur_M_STAT <= M_INFO: print 'NO flank', detectpos, otherinfo, detail2;
	num_n = 10;
	if not detectpos[1][0]==None: 
		curpos = -len(query_pre_suf[1])+detectpos[1][0]
		if cur_M_STAT <= M_INFO: print 'suf', oldstr[curpos:], detectpos[1]
		oldstr = oldstr[:curpos]+('N'*(num_n*len(repPat)))+oldstr[curpos:]
	if not detectpos[0][1]==None:
		curpos = detectpos[0][1]
		if cur_M_STAT <= M_INFO: print 'pre', oldstr[:(curpos+1)], detectpos[0]
		oldstr = oldstr[:(curpos)]+('N'*(num_n*len(repPat)))+oldstr[(curpos):]
	if cur_M_STAT <= M_INFO: print 'n-seq', oldstr
	return oldstr
...
def getUnsymAlignAndHMM(repPat, forw_rerv, repeatFlankLength, hmmoptions, queryrep, commonOptions, otherinfo):
	...
	if commonOptions['isGapCorrection']==0:
		...
		if (len(queryrep)-2*repeatFlankLength)>10:
			unewstr = getAlignment.myUnsymmetricPairAlignment(repPat, queryrep[repeatFlankLength:(len(queryrep)-repeatFlankLength)], forw_rerv, match, mismatch, gap_in_perf, cur_gap_in_read, gap_before_after, mismatchstr=hmmoptions[8][0], mismnum=hmmoptions[8][2]);
		else: unewstr = queryrep
	unewstr = unewstr.replace('-', '')

	if repeatFlankLength>0 and commonOptions['isGapCorrection']>0: unewstr =  queryrep[:repeatFlankLength]+ unewstr + queryrep[(len(queryrep)-repeatFlankLength):]

	unewstr = insert_n_for_flanking(unewstr, rep_predata, rep_sufdata, repeatFlankLength, repPat, otherinfo)
...
def getGene(repeatName, chr, gene_start_end, unique_file_id, analysis_file_id, hgfn, flanking_len, specifiedOptions): #='hg38.fa'
	...
	if flanking_len<min_flank_len: flanking_len=min_flank_len
	interest_region_pos = [[gene_start_end[0]-flanking_len, gene_start_end[0]-1], [gene_start_end[0], gene_start_end[1]], [gene_start_end[1]+1, gene_start_end[1]+flanking_len]]
	my_interest_region = []
...
def getRepeatForGivenGene(commonOptions, specifiedOptions, moreOptions):
        ...
        if (not specifiedOptions.has_key('thread')): logging.info('Running '+get_alg_cmd)
        os.system(get_alg_cmd);
        ...
        predata, mfadata, sufdata = getGene(repeatName, chr, gene_start_end, unique_file_id, analysis_file_id, commonOptions['hgfile'], repeatFlankLength, specifiedOptions)
        rep_predata, rep_mfadata, rep_sufdata = getGene(repeatName, chr, repeat_start_end, unique_file_id, analysis_file_id, commonOptions['hgfile'], repeatFlankLength, specifiedOptions)
        ...
        for line in aligndata:
                ...
                queryrep = '';
                longer = False;
                query_start_ind = None; query_end_ind = None;
                ...
                for n1ind in range(len(numinfo)):
                        ...
                        if pos-1 > repeat_start_end[1] + repeatFlankLength: longer = True;

                if not repeats_dict.has_key(readid):
                   repeats_dict[readid] = [query_start_ind, query_end_ind, aainfo]
                else:
                   if not query_start_ind==None:
                      if repeats_dict[readid][0]==None or repeats_dict[readid][0]>query_start_ind:
                         repeats_dict[readid][0]=query_start_ind
                   if not query_end_ind==None:
                      if repeats_dict[readid][1]==None or repeats_dict[readid][1]<query_end_ind:
        ...
        if handleint:
           ...
           for rk in repeatskeys:
              if repeats_dict[rk][0]==None or repeats_dict[rk][1]==None:
                 repeats.append([False, str(repeats_dict[rk][0])+'-to-'+str(repeats_dict[rk][1]), rk])
                 ids.append(rk)
              else:
                 if repeats_dict[rk][1] - repeats_dict[rk][0]>0:
                    repeats.append([True, repeats_dict[rk][2][repeats_dict[rk][0]:(repeats_dict[rk][1]+1)], rk])
                    ids.append(rk)
                 else:
                    if commonOptions['outlog'] <= M_WARNING:
        ...
        for currep_ind in range(len(repeats)):
                ...
                if len(newstr)<commonOptions['MaxRep']*len_repPat:
                      if currep[0]: 
                         ...
                         newstr, pre0, predstats = getUnsymAlignAndHMM(repPat, forw_rerv, repeatFlankLength, hmmoptions, currep[1], commonOptions, ids[currep_ind])
                      else:
                         if (not specifiedOptions.has_key('thread')):
                            logging.warning('The sequence is partial: '+str(len(newstr))+' '+chr+' '+repeatName+' '+repPat+' '+str(currep[0])+' reads name:'+currep[2]+" "+str(commonOptions['MaxRep'])+" "+str(commonOptions['MaxRep']*len_repPat))
                            if handleint: logging.warning(str(repeats_dict[currep[2]][:2]))
                else: 
                    logging.warning('The sequence is too long: '+str(len(newstr))+' '+chr+' '+repeatName+' '+repPat+' '+str(currep[0])+' reads name:'+currep[2]+" "+str(commonOptions['MaxRep'])+" "+str(commonOptions['MaxRep']*len_repPat))
                    if handleint: logging.warning(str(repeats_dict[currep[2]][:2]))
        ...
        for currep_ind in range(len(repeats)):
                ...
                for i in range(orignial[currep_ind][1]): prestr = prestr + ' ';
...
def getRepeatForKnownGene(commonOptions, specifiedOptions, moreOptions={}):
        if moreOptions.has_key('repeatName'): 
        ...
        if commonOptions['outlog'] <= M_INFO: print 'mgloc', mgloc
